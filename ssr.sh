...
Download_SSR(){
    cd "/usr/local/"
    # 从本地复制 manyuser.zip 文件
    cp /root/manyuser.zip .
    
    [[ ! -e "manyuser.zip" ]] && echo -e "${Error} ShadowsocksR服务端 压缩包 不存在 !" && exit 1
    unzip "manyuser.zip"
...
JQ_install(){
    if [[ ! -e ${jq_file} ]]; then
        cd "${ssr_folder}"
        if [[ ${bit} = "x86_64" ]]; then
            cp /root/jq-linux64 jq
        else
            cp /root/jq-linux32 jq
        fi
        [[ ! -e ${jq_file} ]] && echo -e "${Error} JQ解析器 复制失败，请检查 !" && exit 1
        chmod +x ${jq_file}
